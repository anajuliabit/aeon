...
## Tracked Tokens
Canonical watchlist per `skills/token-alert/SKILL.md` format. Anchors refreshed per 7-17 12:00Z token-alert (**2/12 checks fire — WELL vol spike + GITLAWB 24h rail-breach; 15% rail 3-run streak SNAPS**).

| Token   | CoinGecko ID       | 24h Threshold | Recent Activity |
|---------|--------------------|---------------|-----------------|
| WELL    | moonwell-artemis   | 10%           | 7-17 -7.25% 24h vol $2,008K = **3.29× 5-window baseline** ($610K mean) → **VOLUME SPIKE ALERT fires** (first 3×+ vol since 7-12); participation-vanish state d10 BREAKS with real print returning on fade = **distribution-with-return classification** (heavy prints return on downside after 9 days of vanishing participation, late-cycle distribution shape) |
| MAMO    | mamo               | 15%           | 7-17 +0.14% 24h vol $887K = **0.94× baseline** ($941K mean) = **second-consecutive near-exact-baseline print** = day-7 pause-at-mean sustainable-shape holds cleanly, no distribution risk (weakest 24h print in sweep at 1% of rail, mean-anchored) |
| REPPO   | reppo              | 15%           | 7-17 -12.56% 24h vol $284K = 0.86× baseline ($331K mean) = **near-miss on 15% threshold (84% of rail)** — would fire at 10%; **fade-back-to-trend continuation d3 confirms** — 7-15 -6.74% / 7-16 -3.45% / 7-17 -12.56% cascade; participation doubles vs yesterday but still under-baseline = trend-decay with fresh sellers, no capitulation bid |
| GITLAWB | gitlawb            | 15%           | 7-17 -18.89% 24h vol $236K = 0.60× baseline ($396K mean) → **24h THRESHOLD ALERT fires** (126% of 15% rail = first rail-breach in 4-session arc); **round-trip whipsaw resolves as whipsaw-then-continuation** — 7-14 -11.97%/80% → 7-15 +13.36%/89% → 7-16 -9.70%/65% → 7-17 -18.89%/126% = day-4 breaches to fade side, whipsaw did NOT hold as terminal shape; all 4 sessions printed under-baseline participation (0.63/0.75/0.63/0.60×) = mechanical across full arc |
...
- **GITLAWB whipsaw-then-continuation n=1 CODIFIES** (7-17 close, revises 7-16 "round-trip whipsaw" verdict) — 4-session shape: 7-14 -11.97%/80% rail → 7-15 +13.36%/89% rail → 7-16 -9.70%/65% rail → 7-17 -18.89%/**126% rail-breach** = whipsaw did NOT hold as terminal shape; day-4 breaks to fade side with rail-breach on baseline-only participation (0.60×). All 4 sessions under-baseline (0.63/0.75/0.63/0.60×) = mechanical across full arc, no real-bid ever showed. Revises 7-16 "round-trip closes" call — 3-session round-trip was interim, not terminal. Pattern class updates from "round-trip whipsaw" to **whipsaw-then-continuation** (mechanical whipsaw that resolves into trend-fade continuation on day 4).
