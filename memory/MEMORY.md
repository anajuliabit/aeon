...
skills on cron after 2026-06-01 PR #54. Inbound messaging via Telegram.
soul/ populated 2026-05-25 (ana voice). Reppo-swarm ledger: 14 mints +
29 votes on-chain through 2026-06-02 (today's 1st run added 2 DISLIKE
on epoch-100 HotBot v4 pods 498/499 — first reppo-voter-owned on-chain
votes post-PR #61 chain split). Phase 2 (Pinata pin + platform POST)
end-to-end clean for 5+ consecutive runs since 2026-05-30.
...
- **Close ISS-009 defence-in-depth.** Root cause traced 2026-05-28
  ...
  `.result`). 6 recurrences total — none since 2026-05-30 (4 days clean).
...
- **INDEX bookkeeping flips queued.** ISS-007 (PR #13/#26 merged),
  ...
  ISS-014 (5 consecutive HTTP 200), ISS-015 (PR #56 merged).
...
- **Investigate chain-state-flip anomaly.** 2026-06-02 12:23Z
  ...
  while `gh run view` confirmed workflow exit success. ~5s gap between
...
skipped (Solana PRISM ≠ target). GITLAWB hit −14.93% 2026-06-02 (7bps
under 15% threshold — near-miss flag).
...
- **ISS-015** (high, sandbox-limitation) — PR #56 merged 2026-06-01;
  operator secrets pending (day 5+).
- **ISS-016** (medium, prompt-bug) — own_pod_ids prefetch count=0 for
  8 consecutive runs; voter self-recognizes via ledger cross-ref.
- **ISS-017** (high, prompt-bug) — chain-runner.yml `${{ inputs.chain }}`
  injection at lines 41 + 416. Fix template on disk.
...
- Workflow-injection anti-pattern needs `env:` indirection (canonical
  shape is messages.yml:586-591 after the 2026-04-11 incident).
