...
description: X.AI monthly credit limit hit, blocking 10+ XAI-dependent skills (day 12 of outage as of 2026-06-27)
metadata:
  type: project
---

Team 3a8b4c1e monthly credit limit reached 2026-06-16 (day 12 as of 2026-06-27), blocking:
- token-pick
- agent-buzz
- list-digest
- refresh-x
...
**Partial recovery via prefetched paths.** Skills that run their XAI calls through `scripts/prefetch-xai.sh` *before* the sandboxed Claude step still complete normally — confirmed working through 6-27 for list-digest, narrative-tracker, agent-buzz. The quota error only manifests on in-sandbox calls.

**Cache-quality observation (6-27 agent-buzz):** prefetch script uses `mode:"Latest"` only across 4 sub-queries (chronological tail, not top engagement). Result: uniformly low engagement candidates (0–13 likes max, `followers: null`). Curation degenerated to substantive-claim selection rather than engagement ranking. **Optimization filed (not bug):** dedicate 1 of 4 sub-queries to `mode:"Top"` with `min_likes:50` filter so engagement-ranked candidates surface alongside chronological tail. Current path still produces shippable output.
