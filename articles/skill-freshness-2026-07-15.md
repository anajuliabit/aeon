...
- Dedup status: `FRESHNESS_NO_CHANGE` — fingerprint identical to 2026-07-14 run (sha1=da39a3ee, same FRESHNESS_OK verdict, last run 24h ago, within 7-day re-emit window)
...
|----------|---------------|-----|-------|------|
| cost-report | `cost-report-2026-06-29.md` | ~385 h (16 d) | articles/weekly | STALE (>384 h, >2×192 h) |
| weekly-shiplog | `weekly-shiplog-2026-06-29.md` | ~385 h (16 d) | articles/weekly | STALE (>384 h, >2×192 h) |

**Band escalation since yesterday:** cost-report and weekly-shiplog both crossed from WARN (~360 h on 7-14) into STALE (~385 h today, ≥ 2×192 h weekly threshold). Root cause per MEMORY.md: Monday-slot scheduler miss streak (ISS-027 / rule-5 workflow-file class block). These are scheduler gaps tracked by `skill-health`; this skill reports, `skill-health` tracks and files issues.
