...
with open(f'{base}/cg-trending.json') as f: trend = json.load(f)
with open(f'{base}/cg-markets.json') as f: markets = json.load(f)
with open(f'{base}/dex-trending.json') as f: dex = json.load(f)
...
dex_syms = set()
for p in dex.get('pairs', [])[:120]:
    s = p.get('baseToken', {}).get('symbol', '')
    if s: dex_syms.add(s.upper())
...
for c in markets:
    ...
    score = 0
    breakdown = []
    if p24 > 0: score += 1; breakdown.append('24h+1')
    if p7 > 0: score += 1; breakdown.append('7d+1')
    if p24 > 5 and p7 > 5: score += 2; breakdown.append('both>5%+2')
    if sym in trending_syms: score += 2; breakdown.append('trending+2')
    if vmc >= 0.20: score += 3; breakdown.append('vmc>=0.20+3')
    elif vmc >= 0.10: score += 2; breakdown.append('vmc>=0.10+2')
    if p7 > btc7 and p7 > eth7: score += 2; breakdown.append('RS_BTC+ETH+2')
    if sym in dex_syms: score += 1; breakdown.append('dex+1')
    candidates.append({'sym':sym,'name':name,'price':price,'mcap':mcap,'vol':vol,'p24':p24,'p7':p7,'vmc':vmc,'score':score,'breakdown':breakdown,'id':c.get('id')})
