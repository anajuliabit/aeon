...
def lint(diagram, declared):
    ...
    if subgraph_open and subgraph_open != subgraph_close:
        issues.append("subgraph blocks: " + str(subgraph_open) + " open vs " + str(subgraph_close) + " close")
    for m in re.finditer(r'click ([a-zA-Z0-9_-]+) "([^"]+)"', diagram):
        node, path = m.group(1), m.group(2)
        if node not in declared:
            issues.append("click on undeclared node: " + node)
        full = (DOCS_OUT.parent / path).resolve()
        if not full.exists():
            issues.append("click path missing: " + path)
    return issues

def render(skill_data, depends, consume, reactive_edges, shared, chains, reactive_blocks, mode, prev_state):
    ...
    cur_edges = sorted({(a,b,"dep") for a,b in depends} |
                       ...
                       {(a,b,"rx") for a,b in reactive_edges} |
                       {(a,b,"shr") for a,b,_ in shared})
    node_sha = sha1("\n".join(cur_nodes))
    edge_sha = sha1("\n".join(a + "->" + b + ":" + t for a,b,t in cur_edges))
    ...
    if prev_state:
        ...
        if prev_node_sha != node_sha or prev_edge_sha != edge_sha:
            ...
            if len(cur_nodes) > prev_nodes_total:
                verdict_parts.append("NEW_SKILLS: +" + str(len(cur_nodes) - prev_nodes_total))
            elif len(cur_nodes) < prev_nodes_total:
                verdict_parts.append("RETIRED_SKILLS: -" + str(prev_nodes_total - len(cur_nodes)))
            prev_enabled = prev_state.get("enabled_count", 0)
            if enabled_count != prev_enabled:
                verdict_parts.append("ENABLED_DELTA: " + str(prev_enabled) + "->" + str(enabled_count))
            prev_edges = prev_state.get("edges", {})
            for k, v in edges_total.items():
                pv = prev_edges.get(k, 0)
                if v != pv:
                    verdict_parts.append(k.upper() + "_EDGES: " + str(pv) + "->" + str(v))
    verdict = "ARCHITECTURE_OK" if not verdict_parts else " | ".join(verdict_parts)

    lines = []
    lines.append("# Skill Dependency Graph")
    lines.append("")
    lines.append("_Auto-generated by skill-graph on " + TODAY + " - Mode: `" + mode + "`_")
    lines.append("")
    ...
    if mode != "SKILL_GRAPH_NEW":
        ...
        lines.append("")
        if prev_state:
            lines.append("- Skills: " + str(prev_state.get("skills_total", "?")) + " -> **" + str(len(cur_nodes)) + "**")
            lines.append("- Enabled: " + str(prev_state.get("enabled_count", "?")) + " -> **" + str(enabled_count) + "**")
            prev_edges = prev_state.get("edges", {})
            for k, v in edges_total.items():
                pv = prev_edges.get(k, 0)
                lines.append("- " + k + ": " + str(pv) + " -> **" + str(v) + "**")
        else:
    ...
    for c, count in sorted(cat_counts.items()):
        cat_id = c.replace("-", "_")
        lines.append('  ' + cat_id + '["' + c + '<br/>(' + str(count) + ' skills)"]')
    ...
    for cat in sorted(by_cat.keys()):
        slugs = by_cat[cat]
        lines.append("### " + cat + " (" + str(len(slugs)) + " skills)")
        lines.append("")
        lines.append("```mermaid")
        lines.append("flowchart LR")
        for slug in slugs:
        ...
        for a, b in depends:
            ca = skill_data.get(a, {}).get("category")
            cb = skill_data.get(b, {}).get("category")
            if cb == cat and ca != cat and a in skill_data:
                ext_nodes.add(a)
        for a, b, _ in shared:
            ca = skill_data.get(a, {}).get("category")
            cb = skill_data.get(b, {}).get("category")
            if cb == cat and ca != cat and a in skill_data:
                ext_nodes.add(a)
        slug_ids = {s.replace("-", "_") for s in slugs}
        ext_list = sorted(ext_nodes)[:8]
        for ext in ext_list:
            sid = ext.replace("-", "_")
            if sid not in slug_ids:
                lines.append('  ' + sid + '["' + ext + '"]:::external')
        for a, b in intra_dep.get(cat, []):
            lines.append("  " + a.replace("-","_") + " --> " + b.replace("-","_"))
        seen_shared = set()
        ...
        for slug in slugs:
            ...
            sid = slug.replace("-", "_")
            lines.append("  class " + sid + " " + cls)
            lines.append('  click ' + sid + ' "../skills/' + slug + '/SKILL.md"')
        for ext in ext_list:
            sid = ext.replace("-", "_")
            if sid not in slug_ids:
                lines.append('  click ' + sid + ' "../skills/' + ext + '/SKILL.md"')
    ...
    lines.append("- `-..->` shared-state / reactive (derived from `memory/topics` and `memory/state` references)")
    ...
    lines.append("|---|---|")
    lines.append("| Total skills | " + str(len(skill_data)) + " |")
    lines.append("| Enabled | " + str(enabled_count) + " |")
    lines.append("| Disabled | " + str(len(skill_data) - enabled_count) + " |")
    lines.append("| depends_on edges | " + str(edges_total["depends_on"]) + " |")
    lines.append("| consume edges | " + str(edges_total["consume"]) + " |")
    lines.append("| reactive edges | " + str(edges_total["reactive"]) + " |")
    lines.append("| shared-state edges | " + str(edges_total["shared_state"]) + " |")
    lines.append("")
    lines.append("| Category | Skills |")
    lines.append("|---|---|")
    for c in sorted(by_cat.keys()):
        lines.append("| " + c + " | " + str(len(by_cat[c])) + " |")
    lines.append("")

    footer = ("skills parsed: " + str(len(skill_data)) + " - depends_on: " + str(edges_total["depends_on"]) +
              " - consume: " + str(edges_total["consume"]) + " - reactive: " + str(edges_total["reactive"]) +
              " - shared-state derived: " + str(edges_total["shared_state"]) +
              " - enabled: " + str(enabled_count) + "/" + str(len(skill_data)) +
              " - mode: " + mode)
    lines.append("---")
    lines.append("")
...
def main():
    ...
    if prev is None:
        ...
        mode = "SKILL_GRAPH_OK"
    if cmd == "check":
        out = {"mode": mode, "fingerprint": fp,
               "prev_fp": prev.get("input_fingerprint") if prev else None}
    ...
    if cmd == "render":
        ...
        text, edges_total, verdict, node_sha, edge_sha, footer = render(
            skill_data, depends, consume, reactive_edges, shared, chains, reactive_blocks, mode, prev
        )
        declared = set(d["slug"].replace("-", "_") for d in skill_data.values())
        issues = lint(text, declared)
        result = {
            "mode": mode,
            "fingerprint": fp,
            "verdict": verdict,
