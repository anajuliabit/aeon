...
data, writes ranked recommendations and short-term trade ideas.
...
- **Orchestrator:** `scripts/advisor/run.sh` — prefetch → 5 analysts → debate →
  PM synthesis → short-term trades → POST report + Telegram.
...
- **Short-term trades:** `run.sh` step 5a — 3 stages: (1) deterministic jq
  ...
  conviction-weighted (HIGH = 2× MEDIUM) → `sizeUsd`/`sizePctNet` per trade.
  Surfaced in the Telegram "🎯 Short-term trades" block +
  `report.shortTermTrades`. Complements the daily `token-pick`.
...
- `ADVISOR_DRY_RUN=1 ./scripts/advisor/run.sh` — full pipeline, prints report /
  Telegram to stdout, NO POSTs or Telegram sends.
